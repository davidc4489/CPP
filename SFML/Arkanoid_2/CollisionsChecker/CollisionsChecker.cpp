...
void CollisionsChecker::checkCollision(RenderWindow& window, Ball& ball, Paddle& paddle, Blocks& blocks, 
                                       LifesBoard& lifesBoard, ScoreBoard& scoreBoard, EventChecker& eventChecker, Restarter& restarter) {
    ...
    for (long unsigned int i = 0; i < currentBlocks.size(); i++) {
        ...
        if (collideWithBlock) {
            ...
            scoreBoard.incrementScore(40);
            ball.reverseYDirection();
        }
    }

    if(currentBlocks.size() == 0 && blockCollision == false) {
        eventChecker.button.win = true;
       restarter.restart(blocks);
    }
